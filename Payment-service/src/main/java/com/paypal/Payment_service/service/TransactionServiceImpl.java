...
//        return savedTransaction;
        ...
        try {
            ...
            System.out.println("🔒 Placing hold on sender wallet: " + holdJson);
            System.out.println("hold entity "+ holdEntity);
            ResponseEntity<String> holdResponse = restTemplate.postForEntity(walletServiceUrl + "/hold", holdEntity, String.class);

            System.out.println("🔒 Hold response: status=" + holdResponse.getStatusCode() + " body=" + holdResponse.getBody());
            if (!holdResponse.getStatusCode().is2xxSuccessful() || holdResponse.getBody() == null) {
                throw new RuntimeException("Failed to place hold: status=" + holdResponse.getStatusCode());
            }
        ...
        System.out.println("🚀 Sending Kafka event for transaction: " + savedTransaction);
        kafkaEventProducer.sendTransactionEvent(
                savedTransaction.getId().toString(),
                savedTransaction
        );
        System.out.println("🚀 Kafka event sent for transaction: " + savedTransaction);
        return savedTransaction;
    }
